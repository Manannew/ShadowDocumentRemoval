...
ShadowRemover::ShadowRemover(char* in) {
    ...
    shadowMap = new Mat(height, width, CV_32FC3, CV_RGB(-1, -1, -1));//CV_32FC3 is a three channel matrix of 32-bit floats
...
void ShadowRemover::SaveResults(char* out) {
    ...
    sprintf(buff, "%s_Out.png", out);
    WriteImage(*image, buff);

    sprintf(buff, "%s_ShadowMap.png", out);
